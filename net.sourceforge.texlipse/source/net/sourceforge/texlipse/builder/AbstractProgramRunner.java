...
public abstract class AbstractProgramRunner implements ProgramRunner {
	...
	public static boolean isSaveCommand = false;
	...
	public static int finalErrorLineNumber ;
	...
	public void run(IResource resource) throws CoreException {
		...
		if (PDFAccessibility.isPDFAccessibilityModeOn() && testing) {
			...
			// if(TTSConversion.getDefault().isTTSPowerFlag() && !(severity ==
			// 1) && errorsLine.get(0).intValue() > 0)
			//
			...
			if (!TexParser.fatalErrors) {
				try {
					...
					while ((line = bufferedReader.readLine()) != null) {
						innerloop: for (String defCommand : LatexParser.definedCommandsList) {
					...
					PDFAccessibilityCheck pdfAccessibilityCheck = new PDFAccessibilityCheck(
							strEditor.toString().toLowerCase(), resource);
					...
					// System.out.print("send to server abstract Programmer
					...
					documentServer = "";
					// received data from server in maths equation bind with alt
					// text
			...
			if (!TexParser.fatalErrors) {
				if (!output.contains("<inserted text>") && !output.contains("! LaTeX Error: Missing")) {
					...
					try {
						...
						//if("[tagged]{accessibility}".contains(line))
						//{
		...
		if (!parseErrors(resource, output)) {
			...
			setErrors(true);
			try{
				String errorLineNumber = "0";
				errorLineNumber = (String) (AbstractProgramRunner.finalErrorList.get(0)
						.getLine() == -1 ? "Unknown"
								:String.valueOf(AbstractProgramRunner.finalErrorList.get(0).getLine()));
				finalErrorLineNumber = errorLineNumber.equals("Unknown") ? 0 : AbstractProgramRunner.finalErrorList.get(0).getLine() ;
				
				TTSConversion.getDefault().speak(TTSProperties.SEND_DATA_NEXT);
				Thread.sleep(500);
				TTSConversion.getDefault().speak(finalErrorList.get(0).getMsg() + "at line number" + errorLineNumber);
				isSaveCommand = false;
			}catch(Exception ex){
	...
	protected static void createMarker(IResource resource, final Integer lineNumber, String message, String markerType,
		...
		if (marker == null) {
			try {
				compileErrorStaticList.add(new ParseErrorMessage(lineNr, 0, 0, message, severity));
		...
		// block the warnnings and unnecessary errors to show in the problem
		// window
...
//			}
	...
	public void combineErrorsList() {
		...
		for (ParseErrorMessage error : compileErrorStaticList) { // add run time
																	// errors
			if (error.getSeverity() == 2) {
				errorStack.push(error);
			}
